'use client';

import { useLanguageStore } from '@/lib/stores/languageStore';

type Language = 'ko' | 'en';
type Messages = Record<string, string>;

const messages: Record<Language, Messages> = {
  ko: {
    languageKorean: '한국어',
    languageEnglish: '영어',
    dashboard: '대시보드',
    greeting: '안녕',
    studentFallback: '학생',
    stagePosition: '당신은 지금 Stage {stage}에 있어요',
    journeyLabel: 'SCOPE+ 여정 지도',
    journeyCurrent: '지금 여기',
    journeySubtitle: '대화 기반, 플레이형 탐색',
    journeyPromptStrengths: '나는 어떤 패턴으로 나타날까?',
    journeyPromptCuriosity: '가벼운 마음으로 더 탐색해보고 싶은 것은?',
    journeyPromptOptions: '어떤 학업 경로가 문을 열어주고 나와 잘 맞을까?',
    journeyPromptProof: '어떤 경험이 내 가정을 지지하거나 흔들까?',
    journeyPromptEvolve: '돌아보며 무엇을 유지·조정·삭제할까?',
    journeyPromptStoryboard: '미래 장면을 그려볼까?',
    stage0Subtitle: '편안한 대화를 통해 당신의 관심사와 선호를 함께 알아가요',
    onboardingTag: '함께 시작해요',
    onboardingTitle: '안녕 {name}, 반가워요!',
    onboardingSubtitle: '함께 이야기하며 친구가 되어요. 생각을 나누고, 원하면 자료도 공유해주세요.',
    onboardingGreeting: '안녕, 나는 미래야. 한 걸음씩 함께 해볼게.',
    onboardingIntro: '학교나 활동을 떠올리면, 보통 무엇이 에너지를 주나요?',
    onboardingQuestion1: '공감되는 것을 눌러보세요.',
    onboardingAnswerProblem: '문제 해결',
    onboardingAnswerHelping: '사람 돕기',
    onboardingAnswerCreating: '새로 만들기',
    onboardingAnswerUnsure: '아직 잘 모르겠어요',
    onboardingFollowUp: '고마워요! 한 가지 더 공유할래요?',
    onboardingThanks: '공유해줘서 고마워요. 준비되면 다음 단계로 넘어가요.',
    onboardingPlaceholder: '생각을 적어주거나, 건너뛰어도 좋아요',
    onboardingUploadCta: '자료 업로드 (포트폴리오, 인증서 등)',
    onboardingUploadConfirm: '자료를 잘 받았어요!',
    onboardingKeywords: '{name}\'s Keywords',
    onboardingNextSteps: '곧 만날 여정들',
    onboardingNextStep1: '나를 더 알아가는 시간',
    onboardingNextStep2: '가능성을 탐색하는 놀이',
    onboardingNextStep3: '미래를 그려보는 순간',
    onboardingFinish: '여정 시작하기',
    onboardingChatIntro: '여기서 나눈 이야기는 누구에게도 보이지 않아. 선생님도, 부모님도, 친구도 몰라. 오직 너와 나만의 대화야.\n\n시작하기 전에 지금 어떤 상황인지 조금만 들려줄래? 몇 가지 간단한 질문 해도 괜찮을까?',
    onboardingChatRetry: '지금 연결이 조금 불안정한 것 같아. 다시 한번 말해줄래?',
    onboardingChatNudge: '여기 있어. 조금만 더 이야기해주면 더 잘 도와줄 수 있어.',
    onboardingUploadPrompt: '한 가지 더—어떤 학생들은 아래처럼 참고 자료를 공유하면 도움이 되더라:\n\n📎 커리어넷 검사 결과\n📎 흥미/적성 검사 결과\n📎 학교 상담 노트\n📎 프로젝트 결과물이나 글\n\n있다면 함께 살펴보면서 이야기해볼 수 있어.\n\n없어도 괜찮아. 그냥 계속 대화하면 돼!',
    onboardingUploadButton: '자료 업로드',
    onboardingUploadSkip: '지금은 건너뛰기',
    onboardingUploadFilesLabel: '업로드된 파일 ({count}/6):',
    onboardingUploadDone: '업로드 완료',
    onboardingUploadCancel: '취소',
    onboardingKeywordsEmpty: '대화하면서 키워드가 생성될 거예요...',
    onboardingInfoTitle: '내 정보',
    onboardingInfoSemester: '학기 {value}',
    // Floating Chat
    chatWelcome: '안녕! 언제든지 궁금한 점이나 고민을 물어봐. 함께 이야기하며 길을 찾아갈게.',
    chatSubtitle: '언제든 도와줄게요',
    chatPlaceholder: '무엇이든 물어보세요...',
    chatDisclaimer: '미래와의 대화는 비공개로 진행돼요',
    chatErrorFallback: '조금만 더 이야기해주면 더 잘 도와줄 수 있어.',
    chatErrorRetry: '연결이 불안정해. 다시 한번 말해줄래?',
    onboardingInfoYear: '학년 {value}',
    onboardingInfoStatusPicked: '과목 선택 완료',
    onboardingInfoStatusDeciding: '과목 고민 중',
    onboardingInfoStatusReconsidering: '선택 재고 중',
    onboardingInfoUploads: '업로드 {count}개',
    uploadZonePrompt: '파일을 끌어다 놓거나 눌러서 선택하세요',
    uploadZoneHint: '지원 형식: PNG, JPG, PDF • 최대 {maxFiles}개 • 파일당 {maxSizeMb}MB',
    stage0Name: '자기이해',
    stage0Description: '당신에 대해 알아가기',
    stage1Name: 'Role Roulette',
    stage1Description: '역할 탐색하기',
    stage2Name: '코스 로드맵',
    stage2Description: '과목 설계하기',
    stage3Name: '스킬 번역',
    stage3Description: '성장 여정 그리기',
    stage4Name: '이상형',
    stage4Description: '전문화 좁히기',
    stage5Name: '스토리보드',
    stage5Description: '미래 시각화하기',
    progressLabel: '여정 진행률',
    lockedShort: '잠금',
    start: '시작하기 →',
    complete: '완료됨',
    locked: '이전 단계를 먼저 완료하세요',
    logout: '로그아웃',
    back: '뒤로가기',
    home: '홈',
    settingsOpen: '설정 열기',
    settingsClose: '설정 닫기',
    settingsTitle: '사용자 설정',
    settingsEmailPlaceholder: '이메일 없음',
    settingsAccount: '계정',
    settingsAccountHint: '개인 정보와 여정 설정을 확인하세요.',
    settingsMembership: 'SCOPE+ 멤버',
    settingsStatus: '활성',
    settingsLanguage: '언어',
    settingsLanguageHint: '선호 언어를 전환하세요.',
    // Stage 0
    stage0Question: '당신은 어떤 것들을 잘하나요?',
    stage0OptionAnalytical: '분석적 사고',
    stage0OptionCreative: '창의적 표현',
    stage0OptionEmpathy: '공감 능력',
    stage0OptionOrganization: '체계적 정리',
    stage0Prev: '← 이전',
    stage0Next: '다음 →',
    stage0Complete: '완료',
    stage0ProgressLabel: '질문',
    // Stage 1
    stage1Hint: '← 별로 | 좋아요! | 흥미로워 →',
    stage1SummaryTag: '\uC2A4\uD14C\uC774\uC9C0 1',
    stage1SummaryHeading: '\uB864\uB81C\uB7EC\uC2A4 \uC694\uC57D',
    stage1SummarySubheading: '\uC88B\uC544\uD588\uB358 \uC5ED\uD560\uC744 \uBAA8\uC544\uBD24\uC5B4\uC694. \uC778\uC0C1 \uAE4A\uC5C8\uB358 \uD3EC\uC778\uD2B8\uB97C \uC815\uB9AC\uD558\uACE0 Stage 2\uB85C \uC774\uC5B4\uAC08 \uC218 \uC788\uC5B4\uC694.',
    stage1SummaryEmpty: '\uC544\uC9C1 \uC88B\uC544\uC694 \uD55C \uC5ED\uD560\uC774 \uC5C6\uC5B4\uC694. Stage 1\uC5D0\uC11C \uBA87 \uAC1C\uB97C \uACE8\uB77C\uBD10\uC694.',
    stage1SummaryBackDashboard: '\uB300\uC2DC\uBCF4\uB4DC\uB85C \uB3CC\uC544\uAC00\uAE30',
    stage1SummaryNextStage: '\uB2E4\uC74C \uB2E8\uACC4',
    stage1SummaryHint: '\uC88C\uC6B0\uB85C \uC2A4\uD06C\uB864\uD558\uBA70 \uB2E4\uC2DC \uC0B4\uD3B4\uBCF4\uC138\uC694.',
    stage1SummaryRedo: 'Stage 1 \uB2E4\uC2DC\uD558\uAE30',
    stage1SummaryConfirmTitle: 'Stage 1\uC744 \uB2E4\uC2DC \uD560\uAE4C\uC694?',
    stage1SummaryConfirmBody: '\uC2A4\uC640\uC774\uD504 \uAE30\uB85D\uC774 \uCD08\uAE30\uD654\uB418\uACE0 \uCC98\uC74C\uBD80\uD130 \uB2E4\uC2DC \uC2DC\uC791\uD569\uB2C8\uB2E4.',
    stage1SummaryConfirmCancel: '\uCDE8\uC18C',
    stage1SummaryConfirmConfirm: '\uB2E4\uC2DC\uD558\uAE30',
    stage1SummaryPrev: '\uC774\uC804',
    stage1SummaryNext: '\uB2E4\uC74C',
    stage1SummaryRoleModels: '\uB864\uBAA8\uB378',
    stage1SummaryWhereWork: '\uC77C\uD560 \uC218 \uC788\uB294 \uACF3',
    // Stage 2
    stage2Title: '과목 로드맵',
    stage2Subjects: '과목 목록',
    stage2Anchor: '핵심 과목',
    stage2Signal: '선택 과목',
    stage2AnchorCount: '({count}/6 선택됨)',
    stage2SignalCount: '({count}/6 선택됨)',
    stage2Save: '저장하기',
    // Stage 3
    stage3Title: '스킬 번역',
    stage3Subtitle: '선택한 과목으로 어떤 역량을 키워갈지 함께 이야기해봐요',
    stage3Empty: '안녕! 선택한 과목들을 통해 어떤 스킬을 키우고 싶은지 이야기해봐.',
    stage3Loading: 'AI가 입력 중...',
    stage3Starting: '대화를 시작하고 있어요...',
    stage3Placeholder: '당신의 생각을 자유롭게...',
    stage3Send: '전송',
    stage3Finish: '대화 마치기',
    stage3Retry: '죄송해요, 다시 한번 말씀해주시겠어요?',
    stage3DevSourceLabel: '소스',
    stage3MockLabel: '모의',
    stage3DevTools: '개발 도구',
    stage3MockModeLabel: '모의 모드',
    stage3MockModeHint: '토글하려면',
    toggleOn: '켜짐',
    toggleOff: '꺼짐',
    // Stage 4
    stage4Title: '전문화 토너먼트',
    stage4ComingSoon: 'Stage 4 - 준비 중',
    stage4Subtitle: '최대 3개의 전문화 영역을 선택하세요',
    stage4Selected: '선택됨',
    stage4Complete: '선택 완료',
    stage4Alignment: '{value}% 일치',
    stage4Label: 'Stage 4',
    stage4TournamentTitle: '전문화 이상형',
    stage4Intro: '먼저 전공을 고르고, 그 전공을 제공하는 대학을 비교하세요.',
    stage4MajorTitle: '전공 토너먼트',
    stage4MajorDesc: '관심, 열정, 수강 과목을 바탕으로 한 8개의 후보',
    stage4UniversityTitle: '대학 토너먼트',
    stage4UniversityDesc: '우승 전공을 제공하는 8개 대학',
    stage4Start: '토너먼트 시작',
    stage4RoundLabel: '{round}라운드 / {total}',
    stage4MatchLabel: '{current}경기 / {total}',
    stage4WinnersLocked: '이번 라운드 확정: {value}',
    stage4PickWinner: '승자를 선택하세요',
    stage4PickLabel: '선택',
    stage4MatchScore: '일치 점수: {value}%',
    stage4Careers: '직무: {value}',
    stage4CoreCourses: '핵심 과목: {value}',
    stage4Workload: '학업 스타일: {value}',
    stage4Portfolio: '포트폴리오: {value}',
    stage4Collaboration: '협업: {value}',
    stage4Pace: '페이스: {value}',
    stage4Location: '위치: {value}',
    stage4Scholarships: '장학금: {value}',
    stage4Tuition: '등록금: {value}',
    stage4FinancialAid: '재정 지원: {value}',
    stage4Internships: '인턴십: {value}',
    stage4Selectivity: '선발 난이도: {value}',
    stage4CampusVibe: '캠퍼스 분위기: {value}',
    stage4Housing: '기숙사: {value}',
    stage4Exchange: '교환 프로그램: {value}',
    stage4UniversitySite: '대학 웹사이트',
    stage4WhyMatchup: '추천 이유',
    stage4MatchAlignment: '높은 적합도 ({value}%)',
    stage4MatchCareer: '진로 적합: {value}',
    stage4MatchCourse: '핵심 과목: {value}',
    stage4MatchWorkload: '학업 스타일: {value}',
    stage4MatchCollaboration: '협업: {value}',
    stage4MatchInternships: '인턴십: {value}',
    stage4MatchAid: '지원 강점: {value}',
    stage4MatchVibe: '캠퍼스 분위기: {value}',
    stage4MatchExchange: '교환 프로그램: {value}',
    stage4MatchSelectivity: '선발 난이도: {value}',
    stage4Restart: '토너먼트 다시 시작',
    stage4Undo: '마지막 선택 되돌리기',
    stage4CurrentMajor: '현재 전공: {value}',
    stage4FinalResults: '최종 결과',
    stage4WinningPath: '당신의 우승 경로',
    stage4MajorLabel: '전공',
    stage4UniversityLabel: '대학',
    stage4SaveReturn: '저장 후 대시보드로',
    stage4RunAnother: '다른 토너먼트 진행',
    stage4CombinedTitle: '최종 조합',
    stage4CombinedSummary: '{major} 전공과 {university} 대학의 조합',
    stage4InsightTitle: '추천 근거',
    stage4InsightStrengths: '강점과의 연결: {value}',
    stage4InsightRoles: '관심 역할: {value}',
    stage4ConfidenceLabel: '확신 정도',
    stage4ConfidenceLow: '낮음',
    stage4ConfidenceHigh: '높음',
    stage4ConfidenceValue: '확신 {value}%',
    stage4IntroImageAlt: '토너먼트 시작 이미지',
    stage4ResultImageAlt: '토너먼트 결과 이미지',
    specializationUxDesign: 'UX 디자인',
    specializationDataScience: '데이터 사이언스',
    specializationSocialEntrepreneurship: '소셜 앙트레프러너십',
    specializationContentCreation: '콘텐츠 제작',
    // Stage 5
    stage5Title: '스토리보드',
    stage5When: '언제를 보고 싶나요?',
    stage5Timeline1: '1년 후 (2학년)',
    stage5Timeline3: '3년 후 (대학교)',
    stage5Timeline5: '5년 후 (직장)',
    stage5Generate: '스토리보드 생성하기',
    stage5StoryTitle: '당신의 미래 스토리',
    stage5Finish: '완료하기',
    stage5Scene1Time: '8:00 AM',
    stage5Scene1: '아침: 디자인 프로젝트 작업',
    stage5Scene2Time: '10:00 AM',
    stage5Scene2: '수업: 인간 중심 디자인',
    stage5Scene3Time: '2:00 PM',
    stage5Scene3: '팀 미팅: 소셜 임팩트 프로젝트',
    avatarLabTag: '아바타 랩',
    avatarLabTitle: '아바타 커스터마이징 시스템',
    avatarLabSubtitle: '잠금 해제 가능한 액세서리와 함께 레이어드 아바타 시스템을 테스트하세요. 아래 카드 토글로 진행 상태를 시뮬레이션할 수 있어요.',
    avatarLabSimulateTitle: '카드 수집 시뮬레이션',
    avatarLabSimulateHint: '카드를 토글해 액세서리를 잠금/해제하세요',
    avatarLabCardStage: 'Stage {stage}',
    avatarLabCardsCollected: '{count} / {total}장 수집됨',
    avatarLabHowItWorksTitle: '작동 방식',
    avatarLabHowItWorksItem1: '기본 Mirae PNG는 그대로 유지돼요',
    avatarLabHowItWorksItem2: '액세서리는 기본 이미지 뒤/앞 레이어로 쌓여요',
    avatarLabHowItWorksItem3: '수집 카드에 따라 잠금 해제 규칙이 적용돼요',
    avatarLabHowItWorksItem4: '자동 스타일이 추천 조합을 제안해요',
    avatarLabHowItWorksItem5: '합성된 아바타를 PNG(2x)로 내보내요',
    avatarLabHowItWorksItem6: '설정은 프로필에 저장돼요',
    avatarLabBaseTitle: '기본 Mirae PNG',
    avatarLabBaseCaption: '원본 Mirae 캐릭터 (변경 없음)',
    loginHeroTitle: '진로 탐색 여정을 시작하세요',
    loginTestAccounts: '테스트 계정',
    loginPasswordLabel: '비밀번호',
    loginEmailLabel: '이메일',
    loginButton: '로그인',
    loginLoading: '로그인 중...',
    loginNoAccount: '계정이 없나요?',
    loginSignup: '회원가입',
    loginPasswordValue: '비밀번호: password123',
    loginInvalid: '이메일 또는 비밀번호가 올바르지 않습니다',
    loginPasswordHint: 'Password: password123',
    loginUseTest: '테스트 계정을 사용하세요',
    // Signup
    signupHeroTitle: '새로운 여정을 시작하세요',
    signupDisabledTitle: '회원가입이 비활성화되었습니다',
    signupDisabledBody: '아래 테스트 계정을 사용해주세요.',
    signupNameLabel: '이름',
    signupButton: '회원가입',
    signupLoading: '가입 중...',
    signupExistingAccount: '이미 계정이 있나요?',
    signupLogin: '로그인',
    signupDisabledError: '회원가입은 테스트 계정만 지원합니다.',
  },
  en: {
    languageKorean: 'Korean',
    languageEnglish: 'English',
    dashboard: 'Dashboard',
    greeting: 'Hello',
    studentFallback: 'Student',
    stagePosition: 'You are currently at Stage {stage}',
    journeyLabel: 'SCOPE+ Journey Map',
    journeyCurrent: 'Current stop',
    journeySubtitle: 'Conversation-first, play-based exploration',
    journeyPromptStrengths: 'What makes you, you?',
    journeyPromptCuriosity: 'What do I want to explore more without committing?',
    journeyPromptOptions: 'What academic paths keep doors open and fit me?',
    journeyPromptProof: 'What experiences support or challenge my assumptions?',
    journeyPromptEvolve: 'Based on reflection, what should I keep, adjust, or drop?',
    journeyPromptStoryboard: 'Can I sketch the scenes of my future?',
    stage0Subtitle: 'Let\'s chat and discover together what energizes you, what brings you joy, and what you naturally gravitate toward',
    onboardingTag: 'Let\'s connect',
    onboardingTitle: 'Welcome to Mirae, {name}!',
    onboardingSubtitle: 'Let\'s chat & make friends. This is your private space to think about school and paths.',
    onboardingGreeting: 'Hi, I\'m Mirae. We\'ll take this one step at a time.',
    onboardingIntro: 'When you think about school or activities, what usually gives you energy?',
    onboardingQuestion1: 'Tap what resonates.',
    onboardingAnswerProblem: 'Solving problems',
    onboardingAnswerHelping: 'Helping others',
    onboardingAnswerCreating: 'Creating things',
    onboardingAnswerUnsure: 'I am not sure yet',
    onboardingFollowUp: 'Thanks! Want to share one more?',
    onboardingThanks: 'Appreciate it. Ready to move ahead when you are.',
    onboardingPlaceholder: 'Share a thought, or skip if you like',
    onboardingUploadCta: 'Upload docs (portfolio, certificates, etc.)',
    onboardingUploadConfirm: 'Got your files!',
    onboardingKeywords: '{name}\'s Keywords',
    onboardingNextSteps: 'What\'s ahead',
    onboardingNextStep1: 'Discover more about yourself',
    onboardingNextStep2: 'Explore possibilities playfully',
    onboardingNextStep3: 'Sketch your future moments',
    onboardingFinish: 'Begin your journey',
    onboardingChatIntro: 'No one else can see what we talk about here. Not teachers, not parents, not friends. Just you and me.\n\nBefore we start, I\'d love to know a little about where you\'re at right now. Cool if I ask a few quick questions?',
    onboardingChatRetry: 'Hmm, I\'m having trouble connecting right now. Can you try that again?',
    onboardingChatNudge: 'I\'m here. Want to share a bit more so I can guide you?',
    onboardingUploadPrompt: 'One more thing—some students find it helpful to share things like:\n\n📎 Career.net results\n📎 Interest/aptitude test results\n📎 Notes from school counseling\n📎 Project work or writing samples\n\nIf you have anything like that and want to share, I can help us talk through what you\'re seeing in them.\n\nBut if not, no worries—we can just keep chatting!',
    onboardingUploadButton: 'Upload something',
    onboardingUploadSkip: 'Skip for now',
    onboardingUploadFilesLabel: 'Uploaded files ({count}/6):',
    onboardingUploadDone: 'Done uploading',
    onboardingUploadCancel: 'Cancel',
    onboardingKeywordsEmpty: 'Keywords will appear as we chat...',
    onboardingInfoTitle: 'Your info',
    onboardingInfoSemester: 'Semester {value}',
    // Floating Chat
    chatWelcome: 'Hi! Feel free to ask me anything or share what\'s on your mind. Let\'s explore together.',
    chatSubtitle: 'Here to help anytime',
    chatPlaceholder: 'Ask me anything...',
    chatDisclaimer: 'Your conversations with Mirae are private',
    chatErrorFallback: 'I\'m here. Share a bit more so I can help better.',
    chatErrorRetry: 'Connection seems unstable. Can you try that again?',
    onboardingInfoYear: 'Year {value}',
    onboardingInfoStatusPicked: 'Courses picked',
    onboardingInfoStatusDeciding: 'Still deciding',
    onboardingInfoStatusReconsidering: 'Reconsidering choices',
    onboardingInfoUploads: '{count} document(s) uploaded',
    uploadZonePrompt: 'Drag files here or tap to browse',
    uploadZoneHint: 'Supported: PNG, JPG, PDF • Max {maxFiles} files • {maxSizeMb}MB each',
    stage0Name: 'Strength Discovery',
    stage0Description: 'Learn about yourself',
    stage1Name: 'Role Roulette',
    stage1Description: 'Explore roles',
    stage2Name: 'Course Roadmap',
    stage2Description: 'Design your courses',
    stage3Name: 'Journal Review',
    stage3Description: 'Map your growth journey',
    stage4Name: 'Ideal Type',
    stage4Description: 'Narrow specializations',
    stage5Name: 'Storyboard',
    stage5Description: 'Visualize your future',
    progressLabel: 'Journey progress',
    lockedShort: 'Locked',
    start: 'Start →',
    complete: 'Completed',
    locked: 'Finish the prior stage first',
    logout: 'Sign out',
    back: 'Back',
    home: 'Home',
    settingsOpen: 'Open settings',
    settingsClose: 'Close settings',
    settingsTitle: 'User settings',
    settingsEmailPlaceholder: 'No email on file',
    settingsAccount: 'Account',
    settingsAccountHint: 'Review your profile and journey preferences.',
    settingsMembership: 'SCOPE+ member',
    settingsStatus: 'Active',
    settingsLanguage: 'Language',
    settingsLanguageHint: 'Switch your preferred language.',
    // Stage 0
    stage0Question: 'What are you good at?',
    stage0OptionAnalytical: 'Analytical thinking',
    stage0OptionCreative: 'Creative expression',
    stage0OptionEmpathy: 'Empathy',
    stage0OptionOrganization: 'Organization',
    stage0Prev: '← Back',
    stage0Next: 'Next →',
    stage0Complete: 'Finish',
    stage0ProgressLabel: 'Question',
    // Stage 1
    stage1Hint: '← Not for me | Love it! | Interesting →',
    stage1SummaryTag: 'Stage 1',
    stage1SummaryHeading: 'Role Roulette recap',
    stage1SummarySubheading: 'The roles you liked most are here. Revisit what stood out and carry it into Stage 2.',
    stage1SummaryEmpty: 'No liked roles yet. Go back and explore a few.',
    stage1SummaryBackDashboard: 'Back to dashboard',
    stage1SummaryNextStage: 'Next stage',
    stage1SummaryHint: 'Scroll to review your picks.',
    stage1SummaryRedo: 'Redo Stage 1',
    stage1SummaryConfirmTitle: 'Redo Stage 1?',
    stage1SummaryConfirmBody: 'This will clear your swipes and start over.',
    stage1SummaryConfirmCancel: 'Cancel',
    stage1SummaryConfirmConfirm: 'Redo',
    stage1SummaryPrev: 'Prev',
    stage1SummaryNext: 'Next',
    stage1SummaryRoleModels: 'Role models',
    stage1SummaryWhereWork: 'Where you could work',
    // Stage 2
    stage2Title: 'Course Roadmap',
    stage2Subjects: 'Subjects',
    stage2Anchor: 'Core Courses',
    stage2Signal: 'Elective Courses',
    stage2AnchorCount: '({count}/6 selected)',
    stage2SignalCount: '({count}/6 selected)',
    stage2Save: 'Save selection',
    // Stage 3
    stage3Title: 'Journal Review',
    stage3Subtitle: 'Let\'s talk about what skills you\'ll build from your chosen courses.',
    stage3Empty: 'Hi! Tell me what skills you want to build from your chosen courses.',
    stage3Loading: 'AI is typing...',
    stage3Starting: 'Starting the conversation...',
    stage3Placeholder: 'Share your thoughts...',
    stage3Send: 'Send',
    stage3Finish: 'Finish conversation',
    stage3Retry: 'Sorry—could you say that again?',
    stage3DevSourceLabel: 'Source',
    stage3MockLabel: 'MOCK',
    stage3DevTools: 'Dev tools',
    stage3MockModeLabel: 'Mock mode',
    stage3MockModeHint: 'Press',
    toggleOn: 'ON',
    toggleOff: 'OFF',
    // Stage 4
    stage4Title: 'Specialization Ideal Type',
    stage4ComingSoon: 'Stage 4 - Coming Soon',
    stage4Subtitle: 'Choose up to 3 specialization areas',
    stage4Selected: 'Selected',
    stage4Complete: 'Confirm choices',
    stage4Alignment: '{value}% alignment',
    stage4Label: 'Stage 4',
    stage4TournamentTitle: 'Ideal Type Bracket',
    stage4Intro: 'First, pick the best major. Then compare universities that offer it.',
    stage4MajorTitle: 'Major Ideal Type',
    stage4MajorDesc: '8 candidates based on your interests, passions, and courses.',
    stage4UniversityTitle: 'University Ideal Type',
    stage4UniversityDesc: '8 universities that offer the winning major.',
    stage4Start: 'Start Ideal Type',
    stage4RoundLabel: 'Round {round} of {total}',
    stage4MatchLabel: 'Match {current} of {total}',
    stage4WinnersLocked: 'Winners locked in this round: {value}',
    stage4PickWinner: 'Pick the winner to advance',
    stage4PickLabel: 'Pick',
    stage4MatchScore: 'Match score: {value}%',
    stage4Careers: 'Careers: {value}',
    stage4CoreCourses: 'Core courses: {value}',
    stage4Workload: 'Workload: {value}',
    stage4Portfolio: 'Portfolio: {value}',
    stage4Collaboration: 'Collaboration: {value}',
    stage4Pace: 'Pace: {value}',
    stage4Location: 'Location: {value}',
    stage4Scholarships: 'Scholarships: {value}',
    stage4Tuition: 'Tuition: {value}',
    stage4FinancialAid: 'Financial aid: {value}',
    stage4Internships: 'Internships: {value}',
    stage4Selectivity: 'Selectivity: {value}',
    stage4CampusVibe: 'Campus vibe: {value}',
    stage4Housing: 'Housing: {value}',
    stage4Exchange: 'Exchange: {value}',
    stage4UniversitySite: 'University website',
    stage4WhyMatchup: 'Why this matchup',
    stage4MatchAlignment: 'Strong alignment ({value}%)',
    stage4MatchCareer: 'Career fit: {value}',
    stage4MatchCourse: 'Core course: {value}',
    stage4MatchWorkload: 'Workload: {value}',
    stage4MatchCollaboration: 'Collaboration: {value}',
    stage4MatchInternships: 'Internships: {value}',
    stage4MatchAid: 'Aid strength: {value}',
    stage4MatchVibe: 'Campus vibe: {value}',
    stage4MatchExchange: 'Exchange: {value}',
    stage4MatchSelectivity: 'Selectivity: {value}',
    stage4Restart: 'Restart tournament',
    stage4Undo: 'Undo last pick',
    stage4CurrentMajor: 'Current major: {value}',
    stage4FinalResults: 'Final Results',
    stage4WinningPath: 'Your winning path',
    stage4MajorLabel: 'Major',
    stage4UniversityLabel: 'University',
    stage4SaveReturn: 'Save and return to dashboard',
    stage4RunAnother: 'Run another tournament',
    stage4CombinedTitle: 'Final pairing',
    stage4CombinedSummary: '{major} major at {university}',
    stage4InsightTitle: 'Why this fits',
    stage4InsightStrengths: 'Strengths match: {value}',
    stage4InsightRoles: 'Interested roles: {value}',
    stage4ConfidenceLabel: 'Confidence',
    stage4ConfidenceLow: 'Low',
    stage4ConfidenceHigh: 'High',
    stage4ConfidenceValue: 'Confidence {value}%',
    stage4IntroImageAlt: 'Ideal type start artwork',
    stage4ResultImageAlt: 'Ideal type result artwork',
    specializationUxDesign: 'UX Design',
    specializationDataScience: 'Data Science',
    specializationSocialEntrepreneurship: 'Social Entrepreneurship',
    specializationContentCreation: 'Content Creation',
    // Stage 5
    stage5Title: 'Storyboard',
    stage5When: 'When do you want to see?',
    stage5Timeline1: '1 year from now (Year 2)',
    stage5Timeline3: '3 years from now (University)',
    stage5Timeline5: '5 years from now (Career)',
    stage5Generate: 'Generate storyboard',
    stage5StoryTitle: 'Your future story',
    stage5Finish: 'Finish',
    stage5Scene1Time: '8:00 AM',
    stage5Scene1: 'Morning: Working on a design project',
    stage5Scene2Time: '10:00 AM',
    stage5Scene2: 'Class: Human-centered design',
    stage5Scene3Time: '2:00 PM',
    stage5Scene3: 'Team meeting: Social impact project',
    avatarLabTag: 'Avatar Lab',
    avatarLabTitle: 'Avatar Customization System',
    avatarLabSubtitle: 'Test the layered avatar system with unlockable accessories. Toggle cards below to simulate progress.',
    avatarLabSimulateTitle: 'Simulate Card Collection',
    avatarLabSimulateHint: 'Toggle cards to unlock/lock accessories',
    avatarLabCardStage: 'Stage {stage}',
    avatarLabCardsCollected: '{count} / {total} cards collected',
    avatarLabHowItWorksTitle: 'How it works',
    avatarLabHowItWorksItem1: 'Base Mirae PNG remains unchanged',
    avatarLabHowItWorksItem2: 'Accessories layer behind/in front of base',
    avatarLabHowItWorksItem3: 'Smart unlock rules based on collected cards',
    avatarLabHowItWorksItem4: 'Auto-style suggests recommended combinations',
    avatarLabHowItWorksItem5: 'Export composed avatar as PNG (2x resolution)',
    avatarLabHowItWorksItem6: 'Config saved to your profile',
    avatarLabBaseTitle: 'Base Mirae PNG',
    avatarLabBaseCaption: 'Original Mirae character (unchanged)',
    loginHeroTitle: 'Begin your exploration journey',
    loginTestAccounts: 'Test Accounts',
    loginPasswordLabel: 'Password',
    loginEmailLabel: 'Email',
    loginButton: 'Log in',
    loginLoading: 'Logging in...',
    loginNoAccount: 'No account?',
    loginSignup: 'Sign up',
    loginPasswordValue: 'Password: password123',
    loginInvalid: 'Invalid email or password',
    loginPasswordHint: 'Password: password123',
    loginUseTest: 'Use the test accounts below',
    // Signup
    signupHeroTitle: 'Start a new journey',
    signupDisabledTitle: 'Sign up is disabled',
    signupDisabledBody: 'Please use the test accounts below.',
    signupNameLabel: 'Name',
    signupButton: 'Sign up',
    signupLoading: 'Signing up...',
    signupExistingAccount: 'Already have an account?',
    signupLogin: 'Log in',
    signupDisabledError: 'Sign up is disabled. Please use the test accounts below.',
  },
};

export function useI18n() {
  const { language, setLanguage, toggleLanguage } = useLanguageStore();

  const t = (key: string, vars?: Record<string, string | number>) => {
    const template = messages[language as Language]?.[key] ?? messages.ko[key] ?? key;
    if (!vars) return template;
    return template.replace(/\{(\w+)\}/g, (_, v) => String(vars[v] ?? ''));
  };

  return { t, language, setLanguage, toggleLanguage };
}
